...
class MainWindow(tk.Tk):
    ...
    def __init__(self, controller: AppController):
        ...
        # Установка лого в заголовок окна
        ...
        width, _ = map(int, GEOMETRY_MAIN_WINDOW.split('x'))
    ...
    def _on_window_resize(self, event=None):
        ...
        if new_width != self.current_width:
            ...
            if (
                    hasattr(self.controller, 'ui_handler')
    ...
    def _resize_to_content(self, event=None):
        ...
        if not (
            self.controller.ui_handler and self.controller.ui_handler.canvas
        ):
            return
        ...
        try:
            _, min_height = self.minsize()
        except (tk.TclError, ValueError):
            min_height = 650

        try:
            _, max_height = self.maxsize()
        except (tk.TclError, ValueError):
            max_height = 1500
        ...
        if available_slots != MAX_CONCURRENT_MODULES:
            ...
            current_width, _ = map(int, GEOMETRY_MAIN_WINDOW.split('x'))
